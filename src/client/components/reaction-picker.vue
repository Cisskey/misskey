...
<MkModal ref="modal" :src="src" @click="$refs.modal.close()" @closed="$emit('closed')">
	<div class="rdfaahpb _popup" v-hotkey="keymap">
		...
		<input class="text" ref="text" v-model.trim="text" :placeholder="$t('enterEmoji')" @keyup.enter="reactText" @keyup.esc="close" @input="tryReactText">
	</div>
</MkModal>
</template>
...
export default defineComponent({
	...
	mounted() {
		this.$nextTick(() => {
			this.focus = 0;
			if (this.$store.state.device.enableFocusReactionEmojiInput) this.$nextTick(() => this.$refs.text.focus({ preventScroll: true }));
		});
